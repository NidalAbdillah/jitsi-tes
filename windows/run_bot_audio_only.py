# ==========================================
# Case: Audio Only (Windows)
# Bot join dengan mic aktif, kamera OFF
# ==========================================
...
NUM_SESSIONS = int(os.getenv("NUM_SESSIONS_AUDIO", 5))
...
def start_session(username):
    ...
    result = driver.execute_script('''
        return new Promise(resolve => {
            ...
            .then(tracks => {
                tracks.forEach(track => APP.conference.useAudioStream(track));
...
for i in range(NUM_SESSIONS):
    sessions.append(start_session(f"Aud-{i+1:02d}"))
