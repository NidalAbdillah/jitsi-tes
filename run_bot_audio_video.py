# ==========================================
# 📹🎤 Case: Audio + Video ON
# Bot join dengan kamera dan mikrofon aktif
# Kondisi paling berat untuk server
# ==========================================
...
NUM_SESSIONS     = int(os.getenv("NUM_SESSIONS_AV", 25))
        ...
        return new Promise(resolve => {
            ...
            .then(tracks => {
                ...
                tracks.forEach(track => {
                    if (track.getType() === "video") { APP.conference.useVideoStream(track); types.push("video"); }
                    else if (track.getType() === "audio") { APP.conference.useAudioStream(track); types.push("audio"); }
                });
                resolve("ok: " + types.join("+"));
            })
...
for i in range(NUM_SESSIONS):
    sessions.append(start_session(f"AV-{i+1:02d}"))
